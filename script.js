// =======================================================
// JOGO DAS LETRAS - SCRIPT FINAL, ESTÁVEL E CORRIGIDO
// Autor: Gemini (com correções de robustez e eventos)
// =======================================================
...
// Variáveis Globais de Áudio (escopo global para persistir entre chamadas de função)
let mediaRecorder, audioChunks = [], audioStream = null, timerInterval;

// Variáveis de Jogo
let gameState = {};
let speechReady = false, selectedVoice = null;


// PARTE 2: CONTEÚDO ESTÁTICO DO JOGO
...
const PHASE_2_WORDS = [{ word: 'ABELHA', image: '🐝', vowel: 'A' }, { word: 'ELEFANTE', image: '🐘', vowel: 'E' }, { word: 'IGREJA', image: '⛪', vowel: 'I' }, { word: 'ÔNIBUS', image: '🚌', vowel: 'O' }, { word: 'UVA', image: '🍇', vowel: 'U' }];
const PHASE_3_ENCONTROS = [{ word: 'PEIXE', image: '🐠', encontro: 'EI' }, { word: 'BOI', image: '🐂', encontro: 'OI' }, { word: 'CAIXA', image: '📦', encontro: 'AI' }];
const VOWEL_ENCOUNTERS = ['AI', 'EI', 'OI', 'UI', 'AU', 'EU', 'ÃO', 'ÃE', 'UA', 'ÉU'];
const PHASE_4_WORDS = [{ word: 'BOLA', image: '⚽', options: ['BOLO', 'BALA', 'BULA'] }, { word: 'CASA', image: '🏠', options: ['COPO', 'COLA', 'CAJU'] }, { word: 'DADO', image: '🎲', options: ['DEDO', 'DIA', 'DOCE'] }];
const PHASE_5_WORDS = [{ word: 'BOLO', image: '🎂', syllable: 'LO' }, { word: 'CASA', image: '🏠', syllable: 'SA' }, { word: 'DADO', image: '🎲', syllable: 'DO' }];
...
// PARTE 4: INICIALIZAÇÃO E EVENTOS PRINCIPAIS
document.addEventListener('DOMContentLoaded', initApp);

async function initApp() {
    if (!window.supabase) { alert("ERRO CRÍTICO: Supabase não carregou."); return; }
    initializeSpeech();
    setupAllEventListeners(); // Configura todos os eventos estáticos da página
...
function setupAllEventListeners() {
    // --- Eventos que só precisam ser adicionados UMA VEZ ---
    ...
    document.getElementById('generatePasswordBtn').addEventListener('click', () => {
        ...
        passwordField.type = 'text'; passwordField.value = generateRandomPassword();
    ...
    // ARQUITETURA DE EVENTOS CORRIGIDA:
    // A lógica das abas dos modais é gerenciada por funções específicas (`manageClass`, `showAudioSettingsModal`)
    // para evitar conflitos de múltiplos listeners.
}
...
// (Sem alterações, esta parte estava funcional)
...
// PARTE 6: DASHBOARD DO PROFESSOR E GERENCIAMENTO
...
async function manageClass(classId, className) {
    ...
    // LÓGICA DE EVENTOS ROBUSTA USANDO DELEGAÇÃO
    // Adiciona um único listener no container das abas
    ...
    // Abre na primeira aba por padrão
    const initialTab = document.querySelector('#manageClassModal .tab-btn[data-tab="studentsTab"]');
    showTab(initialTab);
...
async function handleResetStudentPassword(studentId, studentName) { const newPassword = generateRandomPassword(); const confirmed = prompt(`Nova senha para "${studentName}":\n\n${newPassword}\n\nCopie e clique OK.`, newPassword); if (!confirmed) return; try { const hashedPassword = await hashPassword(newPassword); await supabaseClient.from('students').update({ password: hashedPassword }).eq('id', studentId); showFeedback(`Senha de "${studentName}" alterada!`, 'success'); } catch (error) { showFeedback(`Erro: ${error.message}`, 'error'); } }
...
// PARTE 7: ÁUDIO (LÓGICA REVISADA E CORRIGIDA)
...
async function startRecording() {
    const recordBtn = document.getElementById('recordBtn'), stopBtn = document.getElementById('stopBtn'), statusEl = document.getElementById('recordStatus');
    ...
    try {
        ...
        alert("Não foi possível acessar o microfone. Verifique as permissões do navegador e certifique-se de estar em um ambiente seguro (HTTPS ou localhost).");
        statusEl.textContent = 'Falha ao iniciar.';
        recordBtn.disabled = false;
    }
}
function stopRecording() { if (mediaRecorder && mediaRecorder.state === 'recording') { mediaRecorder.stop(); stopTimer(); document.getElementById('recordBtn').disabled = false; document.getElementById('stopBtn').disabled = true; document.getElementById('recordStatus').textContent = 'Parado. Reproduza para conferir.'; } }
...
function showAudioSettingsModal() { const modal = document.getElementById('audioSettingsModal'); const letterSelect = document.getElementById('letterSelect'); let optionsHtml = '<optgroup label="Instruções e Feedbacks">'; for (const key in CUSTOM_AUDIO_KEYS) { optionsHtml += `<option value="${key}">${CUSTOM_AUDIO_KEYS[key]}</option>`; } optionsHtml += '</optgroup><optgroup label="Letras do Alfabeto">'; optionsHtml += ALPHABET.map(letter => `<option value="${letter}">Letra ${letter}</option>`).join(''); optionsHtml += '</optgroup>'; letterSelect.innerHTML = optionsHtml; const tabsContainer = modal.querySelector('.modal-tabs'); if (!tabsContainer.dataset.listener) { tabsContainer.addEventListener('click', (e) => { const button = e.target.closest('.tab-btn'); if (button) showTab(button); }); tabsContainer.dataset.listener = 'true'; } showTab(modal.querySelector('.tab-btn')); showModal('audioSettingsModal'); }
...
// PARTE 10: LOG DE ERROS E RELATÓRIOS
